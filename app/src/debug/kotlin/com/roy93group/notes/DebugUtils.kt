...
object DebugUtils {
    ...
    fun getRandomNote(status: NoteStatus): Note {
        val title = com.roy93group.notes.DebugUtils.getRandomString(10..32)
        ...
        when (type) {
            NoteType.TEXT -> {
                content = com.roy93group.notes.DebugUtils.getRandomString(32..256)
            ...
            NoteType.LIST -> {
                ...
                content = buildString {
                    repeat(size) {
                        appendLine(com.roy93group.notes.DebugUtils.getRandomString(16..128))
                    }
        ...
        val added = com.roy93group.notes.DebugUtils.getRandomDate()
        val modified = com.roy93group.notes.DebugUtils.getRandomDate(added)
    ...
    private fun getRandomString(size: IntRange): String {
        val s = size.random()
        var start = (0 until com.roy93group.notes.DebugUtils.LOREM_IPSUM.length - s).random()
        while (start > 0) {
            if (!com.roy93group.notes.DebugUtils.LOREM_IPSUM[start].isLetter()) {
                break
            }
            start--
        }
        var end = start + s
        while (end < com.roy93group.notes.DebugUtils.LOREM_IPSUM.length) {
            if (!com.roy93group.notes.DebugUtils.LOREM_IPSUM[end].isLetter()) {
                break
            }
            end++
        }
        return com.roy93group.notes.DebugUtils.LOREM_IPSUM.substring(start + 1, end)
